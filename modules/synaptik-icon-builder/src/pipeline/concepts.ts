...
${buildCardPromptLines(chunk, count, paths)}`,
    ...
    for (const card of chunk) {
      ...
      const concepts = (entry?.concepts ?? []).slice(0, count).map((c, idx) => {
        ...
        let visualObject = extractVisualObject(rawVo, preset);
        if (!visualObject && rawVo) {
          visualObject = extractVisualObject(
            rawVo.replace(/^isometric\s+stylized\s+/i, ''),
            preset,
          );
        }
        if (!visualObject || hasAntiPattern(visualObject)) {
          throw new Error(
